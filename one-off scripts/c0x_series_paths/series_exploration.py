...
def write_sorted_histogram(histogram_dict, filename, with_item_size=False):
	...
	if with_item_size:
		sorted_hist_data_as_list = sorted(sorted([(key, value[0], value[1]) for key, value in histogram_dict.items()], key=lambda x: x[0]), key=lambda x: -x[1][0])
	else:
		sorted_hist_data_as_list = sorted(sorted([(key, value) for key, value in histogram_dict.items()], key=lambda x: x[0]), key=lambda x: -x[1])
